...
void clear_list(struct list_head *list){
	...
	spin_lock(&sp_list);     // Entra en la sección crítica
	
	list_for_each_entry_safe(elem, aux, list, links){
		remove_singel_item(elem);
	}
	spin_unlock(&sp_list);   // Sale de la sección critica
}
...
static ssize_t modlist_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
  ...
  if ((*off) > 0) 
    ...
    spin_lock(&sp_list);   // Entra sección crítica
  ...
  *off+=len; 
  ...
  spin_unlock(&sp_list); // Sale sección crítica
